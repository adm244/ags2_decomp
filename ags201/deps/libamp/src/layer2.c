...
int layer2_frame(struct AUDIO_HEADER *header,int cnt, char *buf)
{
int i,s,sb,ch,gr,bitrate,bound = 0;
char (*nbal)[] = NULL,(*bit_alloc_index)[][16] = NULL;
unsigned char allocation[2][32];
unsigned char scfsi[2][32];
REAL scalefactor[2][32][3];
REAL subband_sample[2][32][36];
 ...
 */
	...
	//printout();
//	i = (nch == 2) ? 2 : 1;
	memcpy (buf, sample_buffer, 576 * sizeof(short) * nch);
   buf += 576 * sizeof(short) * nch;
	for (ch=0;ch<nch;ch++) {
		for (sb=0;sb<32;sb++)
...
REAL requantize_sample(unsigned short s4,unsigned short nlevels,REAL c,REAL d,REAL factor)
{
register REAL s,s2,s3;
