/* Modified to execute and compile scripts made with 0.92
 *  THIS IS NOT A FULL BEHAVIOUR RECREATION  */
...
static void scsSymbolInfo(scSymbol * p)
{
	...
	if ((p->adr.flags & adrPOINTER) == adrCS)
		deb0("CS+");
	if ((p->adr.flags & adrPOINTER) == adrDS)
		deb0("DS+");
	if ((p->adr.flags & adrPOINTER) == adrBP)
		deb0("BP+");
...
//other parameters (and after 'this', if it exists).
	if (!sctypIsReference(&sym->type)
		&& sctypGetValType(&sym->type) == valSTRUCT) {
...
// deb("`%c'",*x);
		if (*x == ')')
			break;
		if (x[0] == '.' && x[1] == '.' && x[2] == '.') {	//... encountered- do nothing
			x += 3;				//omit `...'
		} else {
			_value *v;
...
//  deb1("%s,",dx->name);
			if (Add_Params_To_Dict && !dx->name) {
				scget_back(ST);
				serr3(scErr_Operand,
					  "Parameter %d to function '%s' is anonymous", pn,
					  sym->name);}
			//check if such a name is  ( e.g foo(int a,int a);)
			if (Add_Params_To_Dict)
				scdicAddSymbol_Ex(SC_symbols, dx);
		}						//!...
	}
	if (*x != ')')
		serr(scErr_Internal, "Internal compiler error(1)");
...
static void scsymCountAndDestruct(scSymbol * sym)
{
	...
	scsSymbolInfo(sym);
	if (sctypGetValType(&sym->type) == valSTRUCT && !sctypIsReference(&sym->type)) {	//first find destructor, if it exists
		char dname[1024];
		scSymbol *_this;
#if DEBUGLEVEL==0
...
//  params=scvalReadParameters();
			fsym =
				scsFind_Function(dname, _this->type.main, NULL, params,
								 NULL);
			scAssert(scsymCountAndDestruct, fsym);
			scvalCallFunction(fsym, &val, sym, &thisval, NULL, params,
							  false);
...
////scsReadFunctionBody////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
static void scsReadFunctionBody(scSymbol * deli, scSymbol * sym,
								int previous_address, scSymbol * in_struct)
{
	scSymbol *par_symbols;
	if (!sctypIsFunction(&sym->type))
		serr2(scErr_Declaration, "Invalid variable `%s' initialization!",
			  sym->name);
	if (deli)
		serr2(scErr_Declaration, "Function '%s'in function definition",
			  sym->name);
	//here jump
	...
	deb0("+++\nLocal symbols:\n");
	{
		...
		while (p != global_symbols) {
			if (!(p->type.flags & typSTATIC)) {
				...
				p = p->next;
			} else {			//FIXME:make STATIC be the end, if !global_symbols
				if (!global_symbols) {
					global_symbols = p;
					deb0
						("WARNING:no global symbols before first static. UNTESTED!\n");
				} else
					p = p->next;
			}
		}
		...
		while (p) {
			if (p->name[0] != '%' && (p->type.flags & typSTATIC)) {
				char *x =
					scAlloc(strlen(p->name) + strlen(function->name) + 6 +
							3 + 1);
				x[0] = '%';
				strcpy(x + 1, "STATIC");
				strcpy(x + strlen(x), ":");
...
//  Gen_Align();
	...
	scdicRestoreDictionary(SC_symbols, par_symbols,
						   scdicFreeFunctionSymbol);
...
*/
void sciReadDefinition(scSymbol * deli, scSymbol * sym,
					   scSymbol * in_struct)
{
	_value val;
		...
		if (first) {
			if (((sym->type.flags & typTYPE) == typUSERDEFINED) &&
				((sym->type.main->type.flags & typTYPE) == typSTRUCT
				 || (sym->type.main->type.flags & typTYPE) == typSCRIPT)) {	//deb1("here we are %s\n",sym->type.main->name);
				if (!deli && sym->type.main != in_struct
					&& !sym->type.main->parent) {scSymbol *psc;
					if (!
						(psc =
						 scdicFoundSymbol(SC_types->first,
										  sym->type.main->name))) {
...
//     serr2(scErr_Declaration, "Invalid identifier `%s' in declaration",sym->name);
		if (!in_struct)
			prev_sm =
				sctypFindExactFunction(sym->name, SC_symbols->first, NULL,
									   &sym->type);
		else {
			prev_sm =
				sctypFindExactFunction(sym->name, in_struct->type.main,
									   NULL, &sym->type);

			if (!prev_sm)
				serr3(scErr_Declaration,
					  "Function %s is not a member of %s", sym->name,
					  in_struct->name);
		}
		...
		if (sctypIsFunction(&sym->type)) {
			...
			if (prev_sm) {		//function had a prototype

				//TODO:compare declaration
				if (prev_sm->adr.address >= 0)
					serr2(scErr_Declaration, "Symbol `%s' already defined",
						  prev_sm->name);
				//erase sym:
				...
				if (prev_sm != sym)	//let's delete new one
				{
					if (!in_struct) {
						...
						scdicFreeFunctionSymbol(prev_sm);
						prev_sm = NULL;
					} else {
						scdicFreeFunctionSymbol(sym);
						sym = prev_sm;
...
/* Count address of symbol */
			...
			if (deli) {			//stack
				...
				if (prev_sm) {
					if ((prev_sm->adr.flags & adrPOINTER) == adrBP) {
						serr2(scErr_Declaration,
							  "Symbol `%s' already defined",
							  prev_sm->name);
					}
				}
				if (sym->type.flags & typSTATIC) {	//static
					sym->adr.address = act.dataused;
					act.dataused += sctypSizeOf(&sym->type);
				...
				if (sym->type.flags & typSTATIC)
					serr2(scErr_Declaration,
						  "Global symbol `%s' declared as static",
						  sym->name);
		...
		if (lexeq("=")) {
			...
			if (deli && !(sym->type.flags & typSTATIC)) {	//stack
				_value vt1, vt2;
				int i = sctypSizeOf(&sym->type);

				if (i * 4 > act.stacksize)
					act.stacksize = i * 4;	//adjust stack size
				if (i == 4)
				...
				if (sctypGetValType(&sym->type) == valSTRUCT) {
					...
					scAssert(sciReadDefinition, _this);
					{
						...
						if (fsym) {
							...
							scvalCallFunction(fsym, &val, sym, &thisval,
											  NULL, params, false);
							...
							if (Static_Start) {
								...
								oip = act.ip;
								act.ip = Static_Start;
								Gen_Opcode(opvcpuJMP,
										   scvalSetINT(&vt1, oip), NULL);	//complete omit
								act.ip = oip;
							}
						} else
							scFree(params);
...
//     Advance;scget_back(ST);
						...
						if (!fsym)
							serr(scErr_Operand,
								 "Unable to find suitable constructor for initialization");
						scvalCallFunction(fsym, &val, sym, &thisval, NULL,
										  params, false);
//     scvalPopUsedRegisters(regs);
					}
				} else
...
void do_switch(scSymbol * deli)
{
	...
	Advance;
	if (!lexeq("("))
		serr2(scErr_Parse, "`%s' instead of `(' required after switch",
			  ST->lexema);
	scsReadExp(&val, true);
	scvalPutToRegister(&val, FALSE);
	if (!lexeq(")"))
		serr(scErr_Parse, "`)' missing");
	Advance;
...
//          if (!valIsANumber(&val)) serr( "non-constant in case");
			...
			if (count >= ___do_switch_MAX_CASE)
				serr2(scErr_Internal, "Too many cases in switch (max:%d)",
					  ___do_switch_MAX_CASE);
			if (!lexeq(":"))
				serr(scErr_Parse, "`:' missing");
			do {
				Advance;
				...
				if (lexeq("}")) {
					...
					if (!lexeq(";"))
						serr(scErr_Parse, "`;' missing");
					loop = 0;
				} else if (lexeq("case") || lexeq("default")) {
					loop = 0;
				} else {
					scsReadOneStatement(deli);
				}
			} while (loop);
		} else if (lexeq("default")) {
			int loop = 1;
			defaulted = 1;
			Advance;
			if (!lexeq(":"))
			...
			do {
				...
				if (lexeq("}")) {
					...
					loop = 0;
					serr(scErr_Parse,
						 "default expected to be last in switch");
				} else {
					scsReadOneStatement(deli);
				}
			} while (loop);
...
//          if (!valIsANumber(&val)) serr( "non-constant in case");
			...
			ip_table[count] = -1;
			Gen_Opcode(opvcpuJTRUE, &ip_value[count],
					   scvalSetINT(&vt2, k));
			act.ip = k;
			count++;
			do {
			...
			do {
				...
				if (lexeq("}")) {
					...
					serr(scErr_Internal,
						 "Internal compiler error (switch/default)");
				} else {
					scsReadOneStatement(deli);
				}
			} while (loop);
...
//   debif (STALine!=last_line) debprintf("\n>%s\n",STALine);
		...
		while (lexeq("#")) {
			...
			Advance;
			if (ST->whitespaces && (ST->lexema[1] || !(IsSymbol(ST->lexema[0])))) {	//deb0("# line numbering directive\n");
				ST->linedif = atoi(ST->lexema) - ST->act_line - 1;
				Advance;
				if (lexeq("\"!!_internal_Script\""))
					strcpy(ST->name, scScript_Title);
				else
...
//    debprintf("ENTERING!!!\n");
			...
			scdicRestoreDictionary(SC_symbols, prev_symbols,
								   scsymCountAndDestruct);
			if (_fdFS) {
				_value v1, v2;
				Gen_Opcode(opvcpuADD, scvalSetRegister(&v1, valINT, regSP),
						   scvalSetINT(&v2, _fdFS));
			}
			if (old_bp != act.afterbp + _fdFS) {	//deb3("\n!!!WARNING:after {} block afterbp=%d,before=%d,SP change=%d\n\n",act.afterbp,old_bp,_fdFS);
			}
//    act.afterbp=old_bp;
			act.afterbp += _fdFS;
		} else
		...
		if (lexeq("__vcpu")) {
			...
			while (!lexeq(")")) {
				...
				if (!scvalIsImmidiateOfType(&val, valINT))
					serr(scErr_Parse,
						 "__vcpu requires constants as parameters");
//    Gen_Opcode(val.val,NULL,NULL);
			...
			Gen_Opcode(opvcpuFORK, &val, &vt1);
			act.ip = tip;
		} else if (lexeq("import")) {	//scPosition pos;
			int imp = 0;
			char *iname = NULL;
			if (deli)
				serr(scErr_Declaration,
					 "Illegal import declaration inside a function");
			imp = sciReadImport(&iname);

			if (!(sym = sctypGetDefinition())) {
				serr2(scErr_Declaration,
					  "Invalid import declaration at `%s'", ST->lexema);
			}

			if (iname)
				sym->import_name = iname;
...
//    sym->flags^=sym->flags&valBP;//valCPU
			if (!sym->name)
				serr(scErr_Declaration,
					 "Declaration of anonymous variable");
			if (!sciValidIdentifier(sym->name))
				serr2(scErr_Declaration,
					  "Invalid identifier `%s' in declaration", sym->name);
...
//        deb0("end.\n");
				if ((sm2 = scdicFoundSymbol(SC_symbols->first, sym->name))) {	//function had a prototype

					if (sm2->adr.flags & adrIMPORTED)
						serr2(scErr_Parse,
							  "Function `%s' previously declared as imported",
							  sm2->name);

					if (sm2->adr.address != -1)
						serr2(scErr_Parse, "Symbol `%s' already defined",
							  sm2->name);
					//erase sym:
...
//    sciAddImportedFunction(sym,NULL);//it is done while first call to function
			scdicAddSymbol_Ex(SC_symbols, sym);
		} else if (lexeq("export")) {	//scPosition pos;
			//pos=ST->lastpos;
			char name[1024];
			Advance;
			Advance;
			if (!sciValidIdentifier(ST->lexema))
				serr2(scErr_Declaration,
					  "Invalid identifier `%s' in declaration",
					  ST->lexema);
			...
			if (!lexeq(";"))
				serr2(scErr_Parse, parse_error, ST->lexema);
		} else if (lexeq("typedef")) {	//structure def : typedef int fixed
			Advance;
			Advance;
			scget_back(ST);
			if (!(sym = sctypGetDefinition())) {
			...
			if ((sym->type.flags & typTYPE) == typSTRUCT) {
				scFree(sym);
				deb0("Defining struct.\n");
			} else {
				if (!sym->name)
					serr(scErr_Declaration,
						 "type name not specified in type definition");
				if (!sciValidIdentifier(sym->name))
					serr2(scErr_Declaration,
						  "invalid identifier `%s' in declaration",
						  sym->name);
				if (scdicFoundSymbol(SC_types->first, sym->name))
					serr2(scErr_Declaration, "duplicate definition `%s'",
						  sym->name);
		 ...
		 if (lexeq("return")) {
			...
			int ret_struct = 0;
			if (!deli)
				serr(scErr_Parse, "return in not a function");
			Advance;
			...
			if (sctypGetValType(&function->type) != valVOID) {
				...
				if (sctypIsReference(&function->type)) {
					...
					if (sctypGetValType(&function->type) == valSTRUCT) {
						ret_struct =
							sctypSizeOf(&function->type.main->type);
					}
//    scAssert(scsReadOneStatement,deli!=function);
				...
				if (ret_struct) {
					...
					Gen_Opcode(opvcpuMOV,
							   scvalSetRegister(&vt1, valINT, regCX),
							   scvalSetINT(&vt2, ret_struct));
					...
					if (!
						(val.adr.flags & adrREGISTER
						 && val.adr.address == 0 && !(val.adr.flags & adrPOINTER)))	//Output via a0
				{
...
//**************************************************
		if (lexeq("if")) {
			...
			Advance;
			if (!lexeq("("))
				serr2(scErr_Parse, "`%s' instead of `(' required after if",
					  ST->lexema);
			scsReadExp(&val, true);
			if (!lexeq(")"))
				serr(scErr_Parse, "`)' missing");
...
//**************************************************
		if (lexeq("for")) {
			...
			if (!lexeq("("))
				serr2(scErr_Parse,
					  "`%s' instead of `(' required after for",
					  ST->lexema);
			...
			if (val2.type.flags == typINVALID)
				scvalSetINT(&val2, 1);	//always for(..;;..)
...
//**************************************************
		if (lexeq("while")) {
			...
			if (!lexeq("("))
				serr2(scErr_Parse,
					  "`%s' instead of `(' required after while",
					  ST->lexema);
			Gen_Align();
			sip = act.ip;
			scsReadExp(&val, true);
...
//**************************************************
		if (lexeq("do")) {
			...
			Advance;
			if (!lexeq("while"))
				serr2(scErr_Parse, "while expected instead of '%s'",
					  ST->lexema);
			Advance;
			if (!lexeq("("))
				serr2(scErr_Parse,
...
static void scsReadStatement(scSymbol * deli)
{
	...
	if (deli)
		serr2(scErr_Parse, parse_error, " end of file");
...
// 		if (!act.lastInitIp) {	//there was no init
// 			int oip = act.ip;
// 			act.ip = 0;
// #if DEBUGLEVEL<5
...
//HEADer
		{
			...
			//header
			i = CHARS2INT('S', 'e', 'e', 'R');
			push_pmem(&act.final, 4, &i);	//0
			i = CHARS2INT('V', 'C', 'P', 'U');
			push_pmem(&act.final, 4, &i);	//1 int
			i = 0x10000;
			push_pmem(&act.final, 4, &i);	//version,2
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrAllSize,3
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrSymbolOffset - in code,4
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrCodeOffset - in code,5
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrDataOffset - in data,6, NOT USED!
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrConstsOffset,7
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrInitOffset,8
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrCodeSize,9
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrDataSize,10
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrConstsSize,11
			i = act.stacksize;
			push_pmem(&act.final, 4, &i);	//hdrStackSize,12
			act.stacksize = old_stack_size;
			i = 0;
			push_pmem(&act.final, 4, &i);	//hdrImportedOffset,13
			i = CHARS2INT('E', 'O', 'H', '!');
			push_pmem(&act.final, 4, &i);	//EOH!,14
...
// 					p->adr.address + sctypSizeOf(&p->type))
// 					((int *) act.final.mem)[hdrDataSize] =
// 						p->adr.address + sctypSizeOf(&p->type);
// //    scdicFreeFunctionSymbol(p);
...
// 		}
		...
		align_pmem(&act.final);
		{
			...
			while (p) {
				scSymbol *q;
				w = p->next;
				if (!(q = scdicFoundSymbol(SC_symbols->first, p->name))) {	//possibly a member
					...
					do {
						...
						*dot = 0;
						if (!(d = scdicFoundSymbol(d, nam)))
							serr2(scErr_Declaration,
								  "Structure %s does not exist!", p->name);
						*dot = '.';
						if (d->type.flags != typSTRUCT)
							serr2(scErr_Declaration,
								  "%s is not a structure!", p->name);
						nam = dot + 1;
					} while (1);
					if (!d || !(d = scdicFoundSymbol(d->type.main, nam)))
						serr2(scErr_Declaration, "Unable to export %s!",
							  p->name);
					q = d;
				}
				//do the export
...
//  fclose(debfile);
	...
	scdicDeleteDictionary(SC_types);	// woid
	scdicDeleteDictionary(SC_symbols);	// woid
	scdicDeleteDictionary(SC_exports);	// woid
	scdicDeleteDictionary(SC_temporary);	// woid
// scCleanUp(); // woid     
...
{
	while (p) {
		if (sctypIsFunction(&p->type) && p->adr.address < 0
			&& (!(p->adr.flags & adrIMPORTED))) {
			if (s) {
				serr3(scErr_Declaration,
					  "Function `%s.%s' declared and not defined!", s,
					  p->name);} else {
				serr2(scErr_Declaration,
					  "Function `%s' declared and not defined!", p->name);
			}
		}
		p = p->next;
	}
