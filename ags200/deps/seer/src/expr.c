...
static int scsGenStruct(scType * stype, _value * val, scType * temptype,
						bool toStack);
static void scvalPushParameters(_value * fun, _value * params, int *size,
								char **param_end);
static bool scsCallOperator(char *operatorX, _value * val, _value * v1,
							_value * v2, _value * v3, bool CallerIgnore);
...
#define TwoParamInstrNoDouble(isneutral,operand,operstr,opvcpuCODE,neutral,ThisSubExpr,przemienne) \
            ...
            {scvalBothTypes(val,&vt1);                                     \
             ...
             if (((val->type.flags&typUNSIGNED)                          \
                 ||(vt1.type.flags&typUNSIGNED)))                        \
              val->adr.val.uval=val->adr.val.uval    \
                               operand  vt1.adr.val.uval;      \
             else                                                        \
...
{
	_value vt1 =
		{ { /*ADDRESS*/ 0, adrREGISTER, {0}}, { /*type */ typPREDEFINED,
											   NULL, (scSymbol *) valINT},
		NULL };
// scPosition befpos;
	scget_back(ST);
...
//exp1 := lvalue [=] expr | exp2      [=] =: =,+=,-=,*=,/=,%=,&=,^=,|=,<<=,>>=
		{
			...
			if (lexeq("=")) {
				AfterLValueRead valNotConst(val);
				scvalArrayToPointer(&vt1);
				if (scvalGetValType(val) == valSTRUCT) {
					if (scsCallOperator
						("operator=", val, &vt1, NULL, NULL,
						 CallerIgnore)) return 1;
					if (scvalGetValType(&vt1) == valSTRUCT) {
						if (scvalSizeOf(&vt1) != scvalSizeOf(val)) {
							serr(scErr_Operand,
								 "Incompatible struct in copy.");
						} else {
							int size;
...
//              werr(scErr_Operand, "Invalid pointer operation");
				if (scvalGetValType(val) == valSTRUCT) {
					if (!scsCallOperator
						("operator+=", val, &vt1, NULL, NULL,
						 CallerIgnore)) serr3(scErr_Operand,
											  invalid_operand_to, "s",
											  "+=");
				} else {
					if (scvalGetValType(&vt1) == valSTRUCT)
						serr3(scErr_Operand, invalid_operand_to, "s",
							  "+=");
					if (scvalIsImmidiate(&vt1)
						&& (scvalImmidiateValue(&vt1) == 0))
						return 1;
				...
				if (scvalGetValType(val) == valSTRUCT) {
					...
					if (scvalGetValType(&vt1) == valSTRUCT)
						serr3(scErr_Operand, invalid_operand_to, "s",
							  "^=");
					if (scvalIsImmidiateOfType(&vt1, valINT)
						&& (scvalImmidiateValue(&vt1) == 0))
						return 1;
...
//exp5 := exp6 [|] exp5 | exp6
		{
			...
			while (!quit) {
				...
				if (lexeq("|")) {
					quit = false;
					IfNotOperator2Arg("|", 5) {
						TwoParamInstrNoDouble(1, |, "|", opvcpuOR, 0, 5,
											  true);
					}
				}
			}
...
//exp6 := exp7 [^] exp6 | exp7
		{
			...
			while (!quit) {
				...
				if (lexeq("^")) {
					quit = false;
					IfNotOperator2Arg("^", 6) {
						TwoParamInstrNoDouble(1, ^, "^", opvcpuXOR, 0, 6,
											  true);
					}
				}
			}
...
//exp7 := exp8 [&] exp7 | exp8
		{
			...
			while (!quit) {
				...
				if (lexeq("&")) {
					quit = false;
					IfNotOperator2Arg("&", 7) {
						TwoParamInstrNoDouble(1, &, "&", opvcpuAND,
											  0xffffffff, 7, false);
					}
				}
			}
...
//exp8 := exp9 [==] exp8 | exp9      [==] =: ==,!=
		{
			...
			while (!quit) {
				...
				if (lexeq("==")) {
					quit = false;
					IfNotOperator2Arg("==", 8) {
						TwoParamInstr(0, ==, opvcpuCMPE, 0, 8, true,
									  false);
...
//exp9 := exp10 [<] exp9 | exp10      [<] =: <,<=,>,>=
		{
			...
			while (!quit) {
				...
				if (lexeq("<")) {
					quit = false;
					IfNotOperator2Arg("<", 9) {
						TwoParamInstr(0, <, opvcpuCMPL, 0, 9, false,
									  false);
						val->type.flags = typPREDEFINED;
						val->type.main = (scSymbol *)valINT;
					}
				 ...
				 if (lexeq("<=")) {
					quit = false;
					IfNotOperator2Arg("<=", 9) {
						TwoParamInstr(0, <=, opvcpuCMPNG, 0, 9, false,
									  false);
						val->type.flags = typPREDEFINED;
						val->type.main = (scSymbol *)valINT;
					}
				 ...
				 if (lexeq(">")) {
					quit = false;
					IfNotOperator2Arg(">", 9) {
						TwoParamInstr(0, >, opvcpuCMPG, 0, 9, false,
									  false);
						val->type.flags = typPREDEFINED;
						val->type.main = (scSymbol *)valINT;
					}
				 ...
				 if (lexeq(">=")) {
					quit = false;
					IfNotOperator2Arg(">=", 9) {
						TwoParamInstr(0, >=, opvcpuCMPNL, 0, 9, false,
									  false);
						val->type.flags = typPREDEFINED;
						val->type.main = (scSymbol *)valINT;
					}
			...
			while (!quit) {
				...
				if (lexeq("<<")) {
					...
					IfNotOperator2Arg("<<", 10) {
						TwoParamInstrNoDouble(1, <<, "<<", opvcpuSHL, 0,
											  10, false);
					}
				} else
				 if (lexeq(">>")) {
					quit = false;
					IfNotOperator2Arg(">>", 10) {
						TwoParamInstrNoDouble(1, >>, ">>", opvcpuSHR, 0,
											  10, false);
					}
				}
			}
				...
				if (lexeq("+")) {
					...
					IfNotOperator2Arg("+", 12) {
						if (scvalGetValType(val) != valPOINTER) {
							TwoParamInstr(1, +, opvcpuADD, 0, 12, true,
										  false);
						} else {
							_value vl = *val;
							int mul = 4;
							...
							if (scvalGetValType(&vt1) != valINT)
								serr(scErr_Operand,
									 "Invalid 2nd argument to operator+");
				 ...
				 if (lexeq("-")) {
					...
					IfNotOperator2Arg("-", 12) {
						...
						TwoParamInstr(1, -, opvcpuSUB, 0, 12, false,
									  false);
					}
				}
			}
				 ...
				 if (lexeq("%")) {
					...
					IfNotOperator2Arg("%", 12) {
						if (scvalGetValType(val) == valPOINTER)
							serr(scErr_Operand,
								 "Invalid pointer operation");
						TwoParamInstrNoDouble(1, %, "%%", opvcpuMOD,
											  MAXINT, 12, false);
					}
				}
			}
...
//               serr3(scErr_Operand, invalid_operand_to,"","unary !");
				...
				if (scvalIsImmidiate(val)) {
					switch (scvalGetValType(val)) {
					case valDOUBLE:
						val->adr.val.dval = -val->adr.val.dval;
						break;
					case valFIXED:
						val->adr.val.ival = -val->adr.val.ival;
						break;
					case valFLOAT:
						val->adr.val.fval = -val->adr.val.fval;
						break;
					case valCHAR:
					case valSHORT:
					case valINT:
			...
			} else if (lexeq("++"))	//wazne!:(x+(++x)) dla x=1 da 2+2 zamiast 1+2,czyli zle!!!
			{
				Advance;
				Read_MusExp(14, val);
				valNotConst(val);
...
//            countaster--;
				...
				for (; countaster; countaster--) {
					...
					if (
						(val->type.params[0] ==
						 '*'
						 /*||(val->type.params[0]=='['&&val->type.params[1]==']') */
...
//             val->adr.flags-=val->adr.flags&adrPOINTER;
				...
				if (!styp || !styp[0])
					serr(scErr_Parse,
						 "typeof() requires type or variable");
				scvalConstantString(val, styp);

				if (!lexeq(")"))
					serr2(scErr_Parse, "`%s' instead of `)'", ST->lexema);
				Advance;
				return 1;
			} else if (lexeq("sizeof")) {
				int i, j = 0;
				Advance;
				if (!lexeq("("))
					serr2(scErr_Parse, parse_error, ST->lexema);
				...
				Advance;
				return 1;
			} else if (lexeq("(") && (sc = get_definition_on_cast()))	//casting
			{
				if (!lexeq(")"))
					serr2(scErr_Parse, "`%s' instead of `)'", ST->lexema);
				Advance;
				...
				if (lexeq("->") || lexeq(".")) {
					...
					if (lexeq("->")) {
						...
						if (scvalGetValType(val) != valSTRUCT
							&& scvalGetValType(val) != valSCRIPT) {
							serr(scErr_Operand,
								 "not a struct or script in binary `->'");
						}
					}
					if (!w && lexeq(".")) {
						scvalDisplayValue("this(.)=", &thisval);
					...
					if (lexeq("~")) {
						int i;
						Advance;
						for (i = strlen(ST->lexema); i >= 0; i--)
							ST->lexema[i + 1] = ST->lexema[i];
						ST->lexema[0] = '~';
					}
					if (lexeq("operator")) {
...
//           val->type.flags|=typCONST;
						...
						params = scvalReadParameters();
						{
							...
							sprintf(sname,
									"%%s is not a function member of `%s'.%%s",
									father->name);
							sym =
								scsFind_Function(name, find_among, NULL,
												 params, sname);
							scFree(sname);
						}
						if (scriptval.sym)
							scvalCallFunction(sym, val, father, &thisval,
											  &scriptval, params,
											  CallerIgnore);
						else
							scvalCallFunction(sym, val, father, &thisval,
											  NULL, params, CallerIgnore);
						...
						if (!sc)
							serr3(scErr_Operand,
								  "`%s' is not a member of `%s'", name,
								  father->name);

						scAssert(SubExpr::Struct,
								 scvalGetValType(val) != valSCRIPT
								 || (sc->adr.flags & adrSCRIPT));
						...
						if (scvalGetValType(val) == valSCRIPT) {
							...
							scvalSetRegister(&vt1, valINT,
											 scvalFindFreeRegister(val));
						...
						if (scvalGetValType(val) == valSTRUCT) {
							...
							if ((val->adr.flags & adrPOINTER))	//mov a0,[x+v]
							{
								...
								if (val->adr.flags & adrREGISTER) {
									scAssert(sciReadSubExp::operator[],
											 val->type.params
											 && val->type.params[0] ==
											 '*');
									Gen_Opcode(opvcpuADD, val, &vt1);
									...
									serr(scErr_Operand,
										 "Indexing not table nor pointer");
...
//*************** STRING
			if (ST->lexema[0] == '"') {	//String - in consts is stored! DS<0
				char *data;
...
//*************** MEMBER-SYMBOL
				if ((this_symbol.reserved) &&
					(sym =
					 scdicFoundSymbol(this_symbol.type.main->type.main,
									  ST->lexema))) {
				_value vt1, vt2;
				...
				if (!sctypIsFunction(&sym->type)) {
					...
					if (sym->adr.address)
						Gen_Opcode(opvcpuADD,
								   scvalSetRegister(&vt1, valINT,
													val->adr.address),
								   scvalSetINT(&vt2, sym->adr.address));
				}
				val->sym = sym;
				val->type = sym->type;
...
//            scvalGetValType(val);//?
				...
				{
					if (sctypIsFunction(&val->type)) {	//address of function - it'll be changed to int!
//               if (val->adr.flags&adrIMPORTED){
//               }
						serr(scErr_Operand,
							 "At the moment getting the address of a function is not implemented!");
...
//                scSymbol *s=val->sym;
					...
					if (!sctypIsFunction(&sym->type)) {
						if (sctypGetValType(&sym->type) == valSTRUCT) {
							...
							thisval = *val;
							sym1 =
								scsFind_Function("operator()",
												 val->type.main, NULL,
												 params,
												 "Proper `%s' could not be found.%s");
						} else
							serr2(scErr_Operand, "`%s' is not a function",
								  sym->name);
					} else
						sym1 =
							scsFind_Function(sym->name, find_among,
											 SC_symbols->first, params,
											 "`%s' could not be found.%s");

					scvalCallFunction(sym1, val, in_struct, &thisval, NULL,
									  params, CallerIgnore);
				...
				if ((sc = get_definition_on_construct())) {
					...
					deb1("name=%s\n", sc->type.main->name);
					siz =
						scsGenStruct(&sc->type.main->type, val, &sc->type,
									 false);
					val->type = sc->type;
					_this = sc->type.main;
					{
						...
						Advance;
						fsym =
							scsFind_Function(_this->name, _this->type.main,
											 NULL, params,
											 "Constructor `%s' couldn't be found.%s");
						val2 = *val;
						scvalCallFunction(fsym, &val1, sc, &val2, NULL,
										  params, false);
						scvalPopUsedRegisters(regs);
					}
					return 1;
...
//                serr2(scErr_Parse,parse_error,ST->lexema);
					serr2(scErr_Parse, "Invalid expression separator `%s'",
						  ST->lexema);
...
//*********** expr := exp1,expr | exp1
	...
	if (sciReadSubExp(1, val, RequiredExp)) {
		...
		if (lexeq(",")) {
			deb0("Next expr after comma\n");
			Advance;
			return Read_Expression(val, true);
		}
		return 1;
	}
	return 0;
...
//     act.registers[i].used->adr.flags^=ptr;
			Gen_Opcode(opvcpuPUSH, scvalSetRegister(&valt, valINT, i),
					   NULL);
//     act.registers[i].used->adr.flags^=ptr;
			regs[i] = 1;
		}
...
//     ptr=act.registers[i].used->adr.flags&adrPOINTER;
//     act.registers[i].used->adr.flags^=ptr;
			Gen_Opcode(opvcpuPOP, scvalSetRegister(&valt, valINT, i),
					   NULL);
//     act.registers[i].used->adr.flags^=ptr;
		}
	}
...
bool scsTypesExact(scType * t1, scType * t2)
{
	...
	if (sctypIsReference(t1) || sctypIsReference(t2)) {
		...
		if (sctypIsReference(t1))
			sctypGoDeeper(&ct1);
		if (sctypIsReference(t2))
			sctypGoDeeper(&ct2);
	...
	if (gvt1 == gvt2) {
		...
		case valSCRIPT:
			...
			if (sctypSizeOf(t1) != sctypSizeOf(t2))
				return false;
	...
	case valSTRUCT:
		{
			...
			fsym =
				scsFind_Function(_this->name, _this->type.main, NULL,
								 params, NULL);
			scFree(params);
			if (fsym)
				return true;
...
//          thisval=*v;
				scvalCallFunction(fsym, &valx, v->sym, &thisval, NULL,
								  params, false);
				scvalPopUsedRegisters(regs);
				return true;
			}
		...
		case valPOINTER:
			{
				...
				if (sctypGetValType(&ct1) == valVOID
					|| sctypGetValType(&ct2) == valVOID)
					return true;	//void can play both
				return scsTypesExact(&ct1, &ct2);
			}
			return true;
		}
...
bool sctypExactFunction(scType * t1, scType * t2)
{
	...
	while (*x1 != ')') {
		...
		if (!after3dots) {
			...
			if (*x1 == '=')
				v1 = (_value *) strtoul(++x1, &x1, 16);
			if (*x2 == '=')
				v2 = (_value *) strtoul(++x2, &x2, 16);
			if (v1 && v2) {
				if (!v1 || !v2)
					return false;	//all have to have this initializer
		...
		if (!after3dots && !scsTypesExact(&s1->type, &s2->type))
			return false;
	}
	if (*x2 != ')')
		return false;
...
////scsFunctionOK//////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
bool scsFunctionOK(scSymbol * fun, _value * params,
				   bool(*typeCompare) (scType * t1, scType * t2))
{
	bool after3dots = false;
	...
	while ((params[i].adr.flags != (adrIMPORTED & adrTYPEONLY)) ||
		...
		scAssert(scsFunctionOK, *x == ',' || *x == ')');
		if (!after3dots)
			scAssert(scsFunctionOK, xs);
		if (*x == ',')
			x++;
		if (!after3dots && !typeCompare(&xs->type, &params[i].type))
			return false;
		i++;
	}
...
///////////////////////////////////////////////////////////////////////////
int scsFindFunction1Pass(char *name, scSymbol ** found,
						 scSymbol * find_among, scSymbol * also_find,
						 _value * params, bool(*typeCompare) (scType * t1,
															  scType * t2))
{
	int fi = 0, in_also = 0;
	scSymbol *p;
...
//find the exact one
		...
		scsFindFunction1Pass(name, found, find_among, also_find, params,
							 &scsTypesExact);
	if (!fi)
		fi =
			scsFindFunction1Pass(name, found, find_among, also_find,
								 params, &scsTypesAlmostExact);
	if (fi == 1)
		return found[0];
	...
	if (fi > 1) {
		...
		serr4(scErr_Operand,
			  "Unable to decide which of %d candidates for function '%s' to use:%s",
			  scStrdup(func), fi, scStrdup(buf));
	}
...
scSymbol *sctypFindExactFunction(char *name, scSymbol * find_among,
								 scSymbol * also_find, scType * params)
{
	scSymbol *p;
	scSymbol *found = NULL;
	int in_also = 0;
	...
	while (p) {
		if (strcmp(p->name, name) == 0) {
			...
			if (sctypExactFunction(&p->type, params)) {
				if (found)
					serr2(scErr_Operand, "Too many same functions '%s'",
						  name);
				found = p;
				deb0("Yes\n");
			} else
	...
	*size = 0;
	while ((params[i].adr.flags != (adrIMPORTED & adrTYPEONLY)) ||
		...
		if (!after3dots) {
			if (*x == ')')
				serr2(scErr_Operand,
					  "Too many parameters to function `%s'",
					  fun->sym->name);
			xs = (scSymbol *) strtoul(x, &x, 16);	//TODO-check if not number serr(
			if (*x == '=') {
...
//convert params:
	{
		...
		for (z = 0; z <= i; z++) {
			...
			if (z > l)
				params[z] = *def_val[z];
			if (!(after3dots && z + 1 >= after3dots)
				&& !scsTypesExact(&formal[z], &v->type)) {
				if (sctypGetValType(&formal[z]) == valSTRUCT
					&& sctypIsReference(&formal[z])) {	//non-reference should be converted while pushing
					scAssert(scvalPushParameters,
							 scsDoClassCast(&formal[z], v, true));	//create temporary for further delete
				}
			}
		}
	}
		...
		*size += k;
		...
		if (!(after3dots && i + 1 >= after3dots)) {
			...
			if (!scvalSameType(v, scvalSetSymbol(&temp, &xs)))
				serr3(scErr_Operand,
					  "Incompatible type for parameter %d to function %s",
					  i + 1, fun->sym->name);
			if (sctypIsReference(&formal[i])) {	//scvalGetAddress(v,2);
...
/*     if ( !(after3dots&&i+1>=after3dots) &&
     ...
     {//try to cast to structure
      scAssert(scvalPushParameters,scsDoClassCast(&formal[i],v));
		...
		if (
			((after3dots && i + 1 >= after3dots)
			 || !sctypIsReference(&formal[i]))) {_value v1, v2;
			...
			if ((after3dots && i + 1 >= after3dots) ||
				...
				{
					Gen_Opcode(opvcpuMOV,
							   scvalSetRegister(&v1, valINT, regCX),
							   scvalSetINT(&v2, type_size));
					Gen_Opcode(opvcpuSUB,
							   scvalSetRegister(&v1, valINT, regSP),
							   scvalSetRegister(&v2, valINT, regCX));
					scvalSetRegister(&v1, valINT, regSP);
			...
			else {				//dedicated, creation of temporary changed BP, now fix it
...
void scvalCallFunction(scSymbol * sym, _value * val, scSymbol * in_struct,
					   _value * thisval, _value * scriptval,
					   _value * params, bool CallerIgnore)
{
	int siz = 0;
	int ret_struct = 0;
...
//push
	...
	if (!sctypIsReference(&sym->type)
		&& sctypGetValType(&sym->type) == valSTRUCT) ret_struct =
			scsGenStruct(&sym->type.main->type, &ret_struct_val,
						 &sym->type, false);
	...
	if (in_struct) {
		...
		if (!(is->adr.address & adrTYPEONLY))
			is = is->type.main;
		while (is && !in_script) {
			if ((is->type.flags & typTYPE) == typSCRIPT)
				in_script = is;
			is = is->parent;
		}
		if (
			((in_struct->adr.address & adrTYPEONLY)
			 && (in_struct->type.flags & typSCRIPT))
			|| ((in_struct->type.main->adr.address & adrTYPEONLY)
				&& (in_struct->type.main->type.flags & typSCRIPT))
			) {
	...
	if (ret_struct) {			//call to function returning struct
	...
	if (in_script) {
		deb1("call from other script '%s'\n", in_script->name);
		{
			if (!(scriptval && scriptval->sym))
				serr2(scErr_Operand, "Invalid script '%s' access",
					  in_script->name);
...
/* do the call */
	...
	if (sym->adr.flags & adrIMPORTED) {
		...
		if ((sym->adr.address < 0) && (!act.unreal))
			sym->adr.address = -i;
	...
	if (ret_struct) {
		...
		if (sctypIsReference(&val->type)
			|| scvalGetValType(val) != valVOID) {
			if (scvalSizeOf(val) == 8) {	//deb0("dbl\n");
							...
							_value * v2, _value * v3, bool CallerIgnore)
{
	...
	if (scdicFoundSymbol(val->type.main, operatorX)) {	//call it
		int *regs;
		_value thisval;
		...
		fsym =
			scsFind_Function(operatorX, val->type.main, NULL, params,
							 "Suitable `%s' could not be found.%s");
		if (!fsym)
			serr2(scErr_Operand, "Unable to find suitable %s", operatorX);

		scvalCallFunction(fsym, val, father, &thisval, NULL, params,
						  CallerIgnore);
		{
			char buf[1024];
						...
						bool toStack)
{
	...
	if (rv == 4)
		...
		Gen_Opcode(opvcpuSUB, scvalSetRegister(&v1, valINT, regSP),
				   scvalSetINT(&v2, rv));
	scvalSetINT(val, 0);
