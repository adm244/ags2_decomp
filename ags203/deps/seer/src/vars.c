...
bool sciValidIdentifier(char *name)
{
	...
	for (i = 0; Special_Symbols[i]; i++)
		if (strstr(name, Special_Symbols[i]))
			return false;
	for (i = 0; Keywords[i]; i++)
		if (strcmp(name, Keywords[i]) == 0)
			return false;
	for (i = 0; Standard_Types[i].name; i++)
		if (strcmp(name, Standard_Types[i].name) == 0)
			return false;
	return true;
}
...
//   act.registers[il].locked=0;
	...
	serr(scErr_Internal,
		 "Compiler error : expression too complex (register overuse)!");
	return 239;
}
...
int scvalFindFreeDoubleRegister(_value * val)
{
	int il;
	for (il = 2; il < 239; il++) {
		if (act.registers[il].used == NULL
			&& act.registers[il + 1].used ==
			NULL) {act.registers[il].used = val;
...
//   v->where=NULL;
	...
	if ((v->adr.flags & adrPOINTER) == adrDS)
		r = regDS;
	if ((v->adr.flags & adrPOINTER) == adrBP)
		r = regBP;
	if ((v->adr.flags & adrPOINTER) == adrCS)
		r = regCS;
...
{								//int typ;
	if (v1->adr.flags & adrPOINTER || v2->adr.flags & adrPOINTER)
		return;
	if (scvalGetValType(v1) == valDOUBLE) {
		scvalConvToDouble(v2);
	} else if (scvalGetValType(v2) == valDOUBLE) {
...
void scvalConvToInt(_value * val)
{
	...
	if (scvalIsImmidiate(val)) {
		...
		case valCHAR:
			...
			break;
		case valFIXED:
			scAssert(scvalConvToInt(2),
					 (val->type.flags & typTYPE) == typPREDEFINED);
			scvalPutToRegister(val, TRUE);
			val->type.main = (scSymbol *)valINT;
			Gen_Opcode(opvcpuSHR, val,
					   scvalSetImmidiate(&v1, valINT, 16, 0));
			break;
		case valDOUBLE:
		...
		case valFLOAT:
			...
			break;
		default:
			serr2(scErr_Cast, illegal_type_conversion, " to int");
		}

		scAssert(scvalConvToInt,
				 (val->type.flags & typTYPE) == typPREDEFINED);
		scAssert(scvalConvToInt, val->type.main == (scSymbol *)valINT);
	}
...
{
	if (scvalGetValType(val) == valFIXED)
		return;
	...
	if (scvalIsImmidiate(val)) {
		...
		scAssert(scvalConvToFixed,
				 (val->type.flags & typTYPE) == typPREDEFINED);
		val->type.main = (scSymbol *)valFIXED;
	} else {
		_value v1 = { { /*ADDRESS*/ 0, adrREGISTER, {0}
					   }
		, { /*type */ typPREDEFINED, NULL, (void *) valFIXED}, NULL };
		switch ((int) scvalGetValType(val)) {
		case valSHORT:
		case valCHAR:
		case valINT:

			scAssert(scvalConvToFixed(2),
					 (val->type.flags & typTYPE) == typPREDEFINED);
			scvalPutToRegister(val, TRUE);
			Gen_Opcode(opvcpuSHL, val,
					   scvalSetImmidiate(&v1, valINT, 16, 0));
			val->type.main = (scSymbol *)valFIXED;
...
void sciAddImportedFunction(scSymbol * sym, scSymbol * in_struct)
{
	...
	if (sym->import_name)
		use_name = sym->import_name;

	if (sym->adr.address != -1)
		return;
			...
			if ((int) _pmem - (int) act.imports.mem < act.imports.pos) {
				...
				if (strcmp(namer, temp) == 0)
					serr3(scErr_Declaration,
						  "Duplicate import name '%s' for symbol '%s'",
						  temp, sym->name);
				_pmem += 8;
			} else
				namer[0] = 0;
	...
	if (lexeq("(")) {
		...
		if (!scvalIsImmidiateOfType(&val, valINT) || (act.ip != ip))
			serr2(scErr_Declaration,
				  "Constant integer expected instead of %s", ST->lexema);
		imp = val.adr.val.ival;
		if (!lexeq(")"))
			serr2(scErr_Declaration, "`)' expected instead of `%s'",
				  ST->lexema);
	}
	return imp;
}
...
void scvalDisplayValue(char *x, _value * v)
{
	...
	if (v->adr.flags & adrIMMIDIATE)
		deb0("(IMM)");
	if (v->adr.flags & adrREGISTER)
		deb0("(REG)");
	if (v->adr.flags & adrIMPORTED)
		deb0("(IMP)");
	...
	*import_name = NULL;
	...
	while (lexeq("(")) {
		...
		if (isdigit(ST->lexema[0])) {
			...
			if (!scvalIsImmidiateOfType(&val, valINT) || (act.ip != ip))
				serr2(scErr_Declaration,
					  "Constant integer expected instead of %s",
					  ST->lexema);
			if (imp >= 0)
				serr(scErr_Declaration,
					 "Dispatcher descriptor repeated more than once");
			imp = val.adr.val.ival;
			if (imp >= SC_MAX_FUNCTION_DISPATCHERS)
				serr3(scErr_Declaration,
					  "The number %d exceeded the maximum number (%d) of dispatchers",
					  imp, SC_MAX_FUNCTION_DISPATCHERS);
		} else {
			char name[1024];
			int i = 1;
			name[0] = 0;
			if (*import_name)
				serr2(scErr_Declaration,
					  "Function already has an import name '%s'",
					  *import_name);
			while (i) {
				if (lexeq(")"))
					i--;
				...
				if (i) {
					...
					if (ST->newline || ST->whitespaces)
						serr(scErr_Declaration,
							 "Import name cannot contain spaces nor new-line characters (possibly thou hast ')' omited)");
				}
			}
...
void scvalGetAddress(_value * val, int situ)
{
	...
	if (!(val->adr.flags & adrPOINTER)) {
		...
		case 1:
			serr(scErr_Operand, "this is invalid(?)");
		case 2:
			serr(scErr_Operand, "Invalid reference initializer");
		case 3:
			serr(scErr_Operand, "Invalid ADDRESS value");
		}
	}
	if ((val->adr.flags & adrPOINTER) == adrDS)
		r = regDS;
	if ((val->adr.flags & adrPOINTER) == adrES)
		r = regES;
	if ((val->adr.flags & adrPOINTER) == adrBP)
		r = regBP;
	if ((val->adr.flags & adrPOINTER) == adrCS)
		r = regCS;				//seems it's a function
