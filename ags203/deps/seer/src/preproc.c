...
//  if (IsSymbol(ST->lexema[0])) serr( parse_error);
			...
			mchr = ST->act_char;
			if (IsSymbol(nm2[0]) || /*IsSymbol(nm1[0])|| */ !nm1[0]
				|| !nm2[0]) {rm_rets;
				STALine = temp1;
...
//  deb1("<%s>\n",st1);
			while (scget_lexema(ST) && !ST->newline) {
				if (lexeq(nm2)) {	//exchange
					char *cxs;
					if (prev == '#') {
						scSwap(st2, st1, "\"\"", ST->lexstart - 1,
							   ST->act_char);
						cxs = st1;
						st1 = st2;
						...
						if (prev == '+')
							scSwap(st2, st1, nm1 + 1, ST->lexstart,
								   ST->act_char);
						else
							scSwap(st2, st1, nm1, ST->lexstart,
								   ST->act_char);
					}
//       deb1("-%s-",st2);
					cxs = st1;
...
//       deb1("MCR>-%s\n",st1);
			...
			if (dm1 != ')' && dm2 == ')') {
				rm_rets;
				serr2(scErr_Parse, "Too many parameters for macro `%s'",
					  sym->name);}
			if (dm1 == ')' && dm2 != ')') {
				rm_rets;
				serr2(scErr_Parse, "Too few parameters for macro `%s'",
					  sym->name);}
		...
		if (li) {
			...
			if (!lexeq(")")) {
				rm_rets;
				serr2(scErr_Parse, "`)' expected instead of `%s'",
					  ST->lexema);}
		}
	}
// if (st1[strlen(st1)-2]=='\n')st1[strlen(st1)-2]=0;
...
//
#define MAX_DEEP 100
Source_Text *sc_prepro(Source_Text * ST, int until, int skip, char *temp,
					   scDictionary * macros, int init)
{
...
//  debprintf("%s_",ST->lexema);
		if (!ST->lexema[1] && ST->lexema[0] == '#') {
			...
			Advance;
			{
				...
				if (lexeq("!")) {	//dont erase the line - this macro is for line numbering
					if (!skip) {
						int t;
						...
						if (ST->whitespaces
							&& (ST->lexema[1]
								|| !(IsSymbol(ST->lexema[0])))) {
//        difline=ST->act_line-
							t = atoi(ST->lexema);
							Advance;
							strcpy(incfile, ST->lexema);
							if (strcmp(incfile, "\"!!_internal_Script\"")
								== 0)
								in_include--;
...
//        scScript_Author=strdup(s);
					...
					if (!skip) {
						...
						if (IsSymbol(ST->lexema[0])) {
							serr2(scErr_Parse, parse_error,
								  " macro definition");
						}
//         debprintf("\ndefine ");
						if (scdicFoundSymbol(macros->first, ST->lexema)) {
							serr2(scErr_Parse, "Macro `%s' redefined",
								  ST->lexema);}

						if (STALine[ST->act_char] == '(') {	//macro with parameters
...
//         deb2("Macro `%s' with `%s'\n",ST->lexema,tmp);
					...
					Advance;
					if (!skip) {	//debprintf("ifndef %s\n",ST->lexema);
						if (!scdicFoundSymbol(macros->first, ST->lexema)) {	//debprintf("don't skip\n");
							ST = sc_prepro(ST, 1, 0, temp, macros, 0);	//don't skip
						} else
							ST = sc_prepro(ST, 1, 1, temp, macros, 0);	//skip
					...
					if (lexeq("defined")) {
						if (!skip) {
							Advance;
							if (scdicFoundSymbol
								(macros->first, ST->lexema)) ST =
									sc_prepro(ST, 1, _not, temp, macros,
											  0);
							else
								ST =
									sc_prepro(ST, 1, !_not, temp, macros,
						...
						serr2(scErr_Parse, "Unknown #%s directive",
							  ST->lexema);
				} else if (lexeq("endif")) {	//debprintf("endif");
					if (until != 1)
...
//          printf(" include from file %s!\n",sname);
						...
						if (!ST1)
							serr2(scErr_Parse,
								  "Unable to include file `%s'", sname);
					} else {	//Chars
						scSymbol *sym = NULL;
						char *x = NULL;
						...
						if (scInternalHeaders)
							sym =
								scdicFoundSymbol(scInternalHeaders->first,
												 ST->lexema);
						if (sym)
...
//render include
					{
						...
						for (i = prev_lines; i > ST->act_line; i--) {
							ST->lines[i + ST1->num_lines + 1] =
								ST->lines[i];
						}
						for (i = ST->act_line;
							 i <= ST->act_line + ST1->num_lines; i++)
							ST->lines[i + 1] =
								ST1->lines[i - ST->act_line];
						{
							char st[256];
...
//           deb3("Inc[%d]=%d-%d",in_include,ST->act_line+1,included[in_include]);
							if (in_include)
								sprintf(st, "#! %d %s",
										ST->act_line + 1 -
										included[in_include], incfile);
							else
								sprintf(st, "#! %d \"%s\"",
										ST->act_line + 1 -
										included[in_include],
										"!!_internal_Script");
							ST->lines[ST->act_line + ST1->num_lines + 1] =
								scStrdup(st);
							{
								int l;
...
//      debprintf("Komentarz!\n");
			...
			if (!IsSymbol(ST->lexema[0])
				&& (sym = scdicFoundSymbol(macros->first, ST->lexema))) {
			int j = ST->lexstart;	//ST->act_char-strlen(ST->lexema);

			if (sym->type.flags == 1) {
				char *tempo;
				if (STALine[ST->act_char] != '(')
					serr2(scErr_Parse, "Macro `%s' requires parameters",
						  ST->lexema);
				...
				scSwap(temp, STALine, (char *) sym->reserved, ST->lexstart,
					   ST->act_char);
			}
			scFree(STALine);
...
//Load ST:
	...
	while (source[pos]) {
		s[0] = 0;
		{
			int i;
			for (i = 0;
				 source[pos] && source[pos] != 10 && source[pos] != 13;
				 i++, pos++)
				s[i] = source[pos];
		...
		if (ST->num_lines + 2 >= ST->alloced_lines) {
			ST->alloced_lines += 20;
			ST->lines =
				scRealloc(ST->lines, ST->alloced_lines * sizeof(char *));
		}
...
//Load ST:
	...
	while (!pack_feof(file)) {
		...
		if (pack_feof(file)) {
			int i = 0;
			while (s[i] && s[i] != 26 && (s[i] != -1))
				i++;			//debprintf("%c(%d)",s[i],(unsigned int)s[i++]);
			s[i] = 0;
		}
		scCutNL(s);
		ST->num_lines++;
...
scScript scCompile_File(char *filename)
{
	Source_Text *ST;
	volatile scScript Scr = NULL;
	scInit_SeeR();
	scErrorNo = 0;
	scInitAlloc();
	if (!setjmp(act.jump)) {
...
scScript scCompile_Text(char *source)
{
	Source_Text *ST;
	volatile scScript Scr = NULL;
	scInit_SeeR();
