...
static BITMAP *modex_init(int w, int h, int v_w, int v_h, int color_depth)
{
   ...
   if (color_depth != 8) {
      strcpy(allegro_error, get_config_text("Mode-X only supports 8 bit color"));
      return NULL;
   }
   ...
   while ((mode->w != w) || (mode->h != h)) {
      if (!mode->regs) {
	 strcpy(allegro_error, get_config_text("Not a VGA mode-X resolution"));
	 return NULL;
      }
...
static BITMAP *xtended_init(int w, int h, int v_w, int v_h, int color_depth)
{
   ...
   /* check it is a valid resolution */
   if (color_depth != 8) {
      strcpy(allegro_error, get_config_text("Xtended mode only supports 8 bit color"));
      return NULL;
   }

   if ((w != 640) || (h != 400) || (v_w > 640) || (v_h > 400)) {
      strcpy(allegro_error, get_config_text("Xtended mode only supports 640x400"));
      return NULL;
   }
   ...
   __dpmi_int(0x10, &r);
   if (r.h.ah) {
      strcpy(allegro_error, get_config_text("VESA mode 0x100 not available"));
      return NULL;
   }
