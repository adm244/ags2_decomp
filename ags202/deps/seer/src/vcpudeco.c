		 ...
		 if ((opc[0] & 63) < opvcpuMOV)	//1 arg
		{
			...
			if (opc[1] & 1)		//pointer
			{
				...
				case adrSTORECPU:
					if (immi1)	//import
					{
						ReadINT((char *) Ins->code +
								ToCodeINT[hdrImportedOffset] +
								(int) v1.what, imINT1);
						v1.what = &imINT1;
					}
					break;
				case adrES:
					v1.what = (int *)((char *)v1.what + Register[regCS]);
					deb0("ES");
					break;
				case adrDS:
					v1.what = (int *)((char *)v1.what + Register[regDS]);
					deb0("DS");
					break;
				case adrBP:
					v1.what = (int *)((char *)v1.what + Register[regBP]);
					deb0("BP");
					break;
				}
//        debprintf("(value=%d)\n",*((int*)v1.what));
			...
			case opvcpuPUSH:
				deb0("PUSH ");
				PushInt(*((int *) v1.what));
				deb3("(SP=%d,@%d,BP=%d) ", Register[regSP],
					 Register[regSS] + Register[regSP], Register[regBP]);
				break;
			...
			case opvcpuDPUSH:
				...
				PushInt(*(((int *) v1.what)));
				deb2("of %08x%08x", *(((int *) v1.what)),
					 *(((int *) v1.what) + 1));
				deb2(" or %f", *(((int *) v1.what)),
					 *(((int *) v1.what) + 1));
		...
		} else					//2 arg
		{
			...
			if (opc[0] & 64)	//pointer
			{
				...
				case adrSTORECPU:
					...
					break;
				case adrES:
					v1.what = (int *)((char *)v1.what + Register[regES]);
					deb0("ES");
					break;
				case adrDS:
					v1.what = (int *)((char *)v1.what + Register[regDS]);
...
//       switch ( ( opc[1]&((valBP>>2)<<5) ) >>3)//base register
				...
				case adrSTORECPU:
					...
					break;
				case adrES:
					v2.what = (int *)((char *)v2.what + Register[regES]);
					deb0("ES");
					break;
				case adrDS:
					v2.what = (int *)((char *)v2.what + Register[regDS]);
					deb0("DS");
					break;
				case adrBP:
					v2.what = (int *)((char *)v2.what + 
						/*Register[regSS]+ */ Register[regBP]);
						deb1("{%d}", (int) v2.what);
					deb0("BP");
					break;
				}
			...
			case opvcpuCALLEX:	//also add sp,paramcount
				deb0("CALLEX");
				{
					...
					result = callfunc((void *)x, (void *)(Register[regSS] + Register[regSP]), *((int *) v2.what));
					*((int *) Ins->data_stack) = result;

					scActual_Instance = Ins;
			...
			case opvcpuDCALLEX:
				deb0("DCALLEX");
				{
					...
					result = callfunc_d((void *)x, (void *)(Register[regSS] + Register[regSP]), *((int *) v2.what));
					*((double *) Ins->data_stack) = result;
...
*/
					...
					if (*((unsigned int *) v2.what) & scDispatch_Script) {
						scScriptTypeHeader *h =
							*(void **) (Register[regSS] + Register[regSP]);

						deb1("\nCalling another script with %d params\n\n",
							 num);

						scVCall_Instance(h->ins, ((int *) h)[*v1.what / 4],
										 num,
										 ((int
										   *) (Register[regSS] +
											   Register[regSP])) + 1);
						...
						if (*((unsigned int *) v2.what) &
							scDispatch_Double) *((double *) Ins->
												 data_stack) =
								*((double *) h->ins->data_stack);
						else
							*((int *) Ins->data_stack) =
								*((int *) h->ins->data_stack);

					} else
						//imported function
					{
						if ((*((unsigned int *) v2.what)) &
							scDispatch_Void) result = &x;
						else
							result = &Register[0];
						scUserFunctionDispatcher[nr] (result,
													  (void *) x,
													  (int
													   *) (Register[regSS]
														   +
					...
					if (Ins->flags.paused) {	//somebody paused us
						scActual_Instance = NULL;
						actspeed = speed;
						return 0;
					}
...
//        deb1(" Return from func=%f\n",*((double *)Register));
			...
			case opvcpuFIXMUL:
				*((int *) v1.what) =
					fixedmul(*((int *) v1.what), *((int *) v2.what));
				deb0("FIXMUL");
				break;

			case opvcpuFIXDIV:
				*((int *) v1.what) =
					fixeddiv(*((int *) v1.what), *((int *) v2.what));
				deb0("FIXDIV");
				break;
