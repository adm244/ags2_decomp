...
// char *hdrStdLib = "#ifndef SeeR_Internal_StdLib\n" "#include Instances\n"
// /* class string */
// 	"class string{\n"
// 	"public:\n"
// 	" char *cstr;\n"
...
{								//TODO
	...
	if (!what || !what->cstr || !_this->cstr || !with || !with->cstr)
		return 0;
...
// printf("Replace `%s' with `%s'(global=%d)\n",what->cstr,with->cstr,global);
	...
	while ((p = string_find(_this, what)) != -1) {
		...
		dx = p + len2;
		np = onp + dx;
		if (p + len2 > (int) strlen(_this->cstr))
			break;
		_this->cstr = np;
	}
	_this->cstr = onp;
...
// 	scAdd_External_Symbol("string.operator=(char*)",
// 						  string_operatorSET_char);
// 	scAdd_External_Symbol("string.operator+", string_operatorPLUS);
// 	scAdd_External_Symbol("string.operator+(char*)",
// 						  string_operatorPLUS_char);
