...
int ConvOpcodeTo_Double(int opcode)
{
	...
	case opvcpuCMPNL:
		...
		break;
	default:
		serr2(scErr_CodeGen, "Invalid `%s' operand for floating",
			  opcodes_vcpu[opcode].name);
	}
	return opcode;
...
void print_value(_value * v)
{
	if (v->adr.flags & adrPOINTER) {
		...
		if ((v->adr.flags & adrPOINTER) == adrBP)
			adeb0("BP+");
		if ((v->adr.flags & adrPOINTER) == adrES)
			adeb0("ES+");
	}
	if (v->adr.flags & adrREGISTER) {
		if (v->adr.address == regCS) {
			adeb0("CS");
		} else if (v->adr.address == regDS) {
			adeb0("DS");
		} else if (v->adr.address == regES) {
			adeb0("ES");
		} else if (v->adr.address == regSS) {
			adeb0("SS");
		} else if (v->adr.address == regIP) {
			adeb0("IP");
		} else if (v->adr.address == regSP) {
			adeb0("SP");
		} else if (v->adr.address == regBP) {
			adeb0("BP");
		} else if (v->adr.address == regCX) {
			adeb0("CX");
		} else
//    if (v->adr.address==regTHIS) {deb0("THIS");} else
		...
		} else if (scvalGetValType(v) == valDOUBLE) {
...
{
	if ((it == PUSHAnything) && (scvalGetValType(v) == valDOUBLE)
		&& (scvalIsImmidiate(v))) {
...
//DELS pv1=*v1;
		...
		// 	switch (opcode) {
		// 	case opvcpuPUSH:
		// 		return opvcpuPUSHADR;
		// 	default:
		// 		serr2(scErr_CodeGen,
		// 			  "Code generation error for %s with ADDRESS value",
		// 			  opcodes_vcpu[opcode].name);
		// 	}
		// }
		if (scvalGetValType(v1) == valSHORT
			&& (v1->adr.flags & adrPOINTER)) scvalPutToRegister(v1, TRUE);
		...
		if (scvalSizeOf(v1) == 8) {
			...
			default:
				serr2(scErr_CodeGen,
					  "Code generation error for %s with double",
					  opcodes_vcpu[opcode].name);
			}
		}
	}
...
//   case opvcpuPUSH:return opvcpuPUSHADR;
		...
		// if ((v1->type.flags & typUNSIGNED)
		// 	|| (v2->type.flags & typUNSIGNED)) {_value v;
		// 	switch (opcode) {
		// 	case opvcpuCMPG:
		// 	case opvcpuCMPNG:
		// 	case opvcpuCMPL:
		...
		// 	case opvcpuMOD:
		// 	case opvcpuSHR:
		// 		Gen_Opcode(opvcpuOPTIONS,
		// 				   scvalSetINT(&v, optionsvcpuUNSIGNED), NULL);
...
//DELS pv1=*v1;pv2=*v2;
		if (opcode != opvcpuDFLT &&
			...
			opcode != opvcpuFIXDBL &&
			opcode != opvcpuDFIX &&
			opcode != opvcpuDINT && opcode != opvcpuCMOV && opcode != opvcpuWMOV) {	//Conversion allowed
...
//         deb1("Now it is of type:%s\n",ValTypeStr(t1));
		...
		if (opcode == opvcpuJFALSE || opcode == opvcpuJTRUE) {	//optimize JFALSE0,x->JMP x, JTRUE 0,x ->nothing
...
//2-st opcode
		...
		serr2(scErr_CodeGen, "Code generation error %s",
			  "for more than 2 opcodes");
	return;
}
