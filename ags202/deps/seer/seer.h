...
#define scDispatch_Member       ((unsigned)1<<27)
	typedef void (*scFunctionDispatcher) (int *result, void *function,
										  int *params, int paramcount,
										  /*void *_this, */
										  unsigned int options);
	void scStandardDispatcher(int *result, void *function, int *params,
							  int paramcount, unsigned int options);
	void scCDispatcher(int *result, void *function, int *params,
					   int paramcount, unsigned int options);
...
/**************************Instances**************************************/
	...
	EXTERN int abort_executor;
	EXTERN int aborted_ip;
...
/**************************MULTITASKING**********************************/
	...
	int scVLaunch_Instance(scInstance * inst, int spd, int address,
						   int paramc, int *params);
	//paramc is size of params in bytes!
...
//returns number of replaces
	int string_replace(scintString * _this, scintString * what,
					   scintString * with, bool global);
	  ...
	  public:
		...
		seerScript():script(NULL) {
			...
			case scriptLoad:
				...
				break;
				case scriptCompileText:script = scCompile_Text(text);
				break;
				case scriptCompileFile:script = scCompile_File(text);
				break;
		}} ~seerScript() {		/*free(); */
		}
		...
		void free(void) {
			...
			script = scCompile_File(filename);
			return scErrorNo;
		} int compileText(char *text) {
			script = scCompile_Text(text);
			return scErrorNo;
