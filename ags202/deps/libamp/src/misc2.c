	 ...
	 */
			...
			while (l<non_zero[ch]) {
				for (window=0;window<3;window++) {
					int scalefac=scalefac_s[gr][ch][sfb][window];
					int subblock_gain=info->subblock_gain[gr][ch][window];
					REAL a=fras_s(global_gain,subblock_gain,scalefac_scale,scalefac);
					for (i=0;i<window_len;i++) {
						xr[ch][0][t_reorder[header->ID][sfreq][l]]=fras2(is[ch][l],a);
						l++;
					}
...
static int min_cycles = 99999999;
        ...
        __asm__(".byte 0x0f,0x31" : "=a" (cnt1), "=d" (cnt4));
			...
			{
				register REAL Mi = fras2(is[0][l],a[0]);
				register REAL Si = fras2(is[1][l],a[1]);
				register REAL tmp = i_sq2;
				xr[0][0][l]=(Mi+Si)*tmp;
				xr[1][0][l]=(Mi-Si)*tmp;
			}
				...
				for (window=0;window<3;window++) {
					...
					if (t_s[sfb] < isbound[window]) {
						...
						if (ms_flag) {
							...
							a[1]=fras_s(global_gain[1],subblock_gain[1],scalefac_scale[1],scalefac[1]);
							for (i=0;i<window_len;i++) {
								register REAL tmp1=fras2(is[0][l],a[0]);
								register REAL tmp2=fras2(is[1][l],a[1]);
						...
						a[0]=fras_s(global_gain[0],subblock_gain[0],scalefac_scale[0],scalefac[0]);
						for (i=0;i<window_len;i++) {
							register REAL ftmp = fras2(is[0][l], a[0]);
...
int i;
	for (i=0;i<8192;i++)
		t_43[i]=(REAL)pow((REAL)i,1.33333333333f);
}
